...
export default function ProfilePage() {
  ...
  const displayName = user?.actor?.displayName || user?.actor?.person?.email || 'Usuario'
  const email = user?.actor?.person?.email || ''
  const phone = user?.actor?.person?.phone || ''
  const person = user?.actor?.person
    ...
    if (isEditing) {
      try {
        ...
        const updateData = {
          ...
          documentNumber: editedUser.documentNumber,
        }

        // Actualizar la persona a través del actor
        const response = await api.patch(`/persons/${user.actor?.person?.id}`, updateData)
        if (response.status === 200) {
        ...
        <CardContent className="text-center p-8">
          ...
          <div className="flex items-center justify-center">
            <Avatar className="h-20 w-20">
              {user && displayName ? (
                <AvatarImage
                  src={`https://ui-avatars.com/api/?name=${encodeURIComponent(displayName)}&background=purple&color=white`}
                  alt={displayName}
                />
              ) : (
          ...
          <div className="mt-6 text-left">
            ...
            <p className="mt-2 text-sm text-gray-600 dark:text-gray-400">
              Hola, soy {editedUser.firstName} {editedUser.lastName}, una empresa de internet apasionada por la tecnología.
            ...
            <div className="mt-4 space-y-2">
              <div className="flex items-center">
                ...
                {isEditing ? (
                  ...
                  <span className="text-sm text-gray-600 dark:text-gray-400">{editedUser.firstName}</span>
                )}
              </div>
              <div className="flex items-center mt-2">
                ...
                {isEditing ? (
                  ...
                  <span className="text-sm text-gray-600 dark:text-gray-400">{editedUser.lastName}</span>
                )}
              </div>
              <div className="flex items-center">
              ...
              <div className="flex items-center">
                <strong className="text-gray-900 dark:text-gray-100 w-40">Documento:</strong>
                {isEditing ? (
                  <Input
                    value={editedUser.documentNumber}
                    onChange={(e) => setEditedUser((prev) => ({ ...prev, documentNumber: e.target.value }))}
                    className="flex-grow"
                  />
                ) : (
                  <span className="text-sm text-gray-600 dark:text-gray-400">{editedUser.documentNumber}</span>
                )}
              </div>
              <div className="flex items-center">
                <strong className="text-gray-900 dark:text-gray-100 w-40">Perfil:</strong>
                <span className="text-sm text-gray-600 dark:text-gray-400">
                  {user.roles && user.roles.length > 0 ? user.roles[ 0 ].name : "Sin rol asignado"}
                </span>
              </div>
            </div>
          </div>

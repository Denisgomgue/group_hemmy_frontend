...
export default function LoginForm() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            // 1. Hacer login (el backend guarda el token en cookies HTTP-only)
            const loginResponse = await api.post("/auth/login", {
                email: email.trim().toLowerCase(),
                password: password.trim()
            })

            // El backend devuelve: { message, user: { id, email, actorId } }
            // El token se guarda automáticamente en cookies HTTP-only por el backend

            // 2. Obtener perfil completo del usuario con las relaciones
            ...
            if (error.response) {
                if (error.response.status === 401) {
                    ...
                    toast.error("Usuario no encontrado. Verifique su email.")
                } else {

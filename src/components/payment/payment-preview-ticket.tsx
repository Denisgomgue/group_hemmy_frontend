            ...
            <div className="p-6 space-y-6">
                ...
                <div className="space-y">
                    <div className="flex justify-center">
                        ...
                        <span className="text-violet-900 text-lg font-bold">{payment.client?.plan?.name || 'Plan no especificado'}</span>
                    </div>
                </div>
                {/* Detalles de pago */}
                <div className="space-y-4">
                    <div className="flex justify-between">
                        ...
                        <span className="text-gray-900">
                            {payment.paymentDate ? format(new Date(payment.paymentDate), "dd-MM-yyyy", { locale: es }) : 'Fecha no especificada'}
                        </span>
                    </div>

                    <div className="flex justify-between">
                        <span className="text-gray-500 uppercase text-sm">Método de pago</span>
                        <span className="text-gray-900">{payment.paymentType ? getPaymentTypeLabel(payment.paymentType) : 'Método no especificado'}</span>
                    </div>

                    <div className="flex justify-between">
                        <span className="text-gray-500 uppercase text-sm">Estado</span>
                        <span className={payment.status === 'PAYMENT_DAILY' ? 'text-green-500' : 'text-red-500'}>

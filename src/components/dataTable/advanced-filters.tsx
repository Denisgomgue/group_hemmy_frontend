...
interface FilterGroup {
    id: string;
    label: string;
    type: 'checkbox' | 'slider' | 'checkbox-group' | 'date-range';
...
export function AdvancedFilters({ groups, onFiltersChange, initialFilters = {} }: AdvancedFiltersProps) {
    ...
    const handleDateRangeChange = (groupId: string, field: 'from' | 'to', date?: Date) => {
        const newFilters = {
            ...filters,
            [ groupId ]: {
                ...(filters[ groupId ] || {}),
    ...
    const renderFilterGroup = (group: FilterGroup) => {
        switch (group.type) {
            ...
            case 'date-range':
                const dateRange = filters[ group.id ] || {};
                return (
                    <div className="space-y-4">
                        <div className="space-y-2">
                            <Label htmlFor={`${group.id}-from`}>Desde</Label>
                            <DatePicker
                                placeholder="Fecha desde"
                                initialValue={dateRange.from}
                                onSelectedChange={(date) => handleDateRangeChange(group.id, 'from', date)}
                            />
                        </div>
                        <div className="space-y-2">
                            <Label htmlFor={`${group.id}-to`}>Hasta</Label>
                            <DatePicker
                                placeholder="Fecha hasta"
                                initialValue={dateRange.to}
                                onSelectedChange={(date) => handleDateRangeChange(group.id, 'to', date)}
                            />
                        </div>
                    </div>
                );
